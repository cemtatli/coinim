...
export default function WatchList() {
  ...
  const deleteCoin = async (passedid) => {
    try {
      ...
      toast.success(t("watchList.successMessage"));
    } catch (e) {
    ...
    <div className="fluid">
      {coins?.length === 0 ? (
        <div className="flex w-full flex-col items-center justify-center gap-4 text-center">
          <p className="w-full text-start text-sm">{t("watchList.emptyListMessage")}</p>
        ...
        <table className="w-full border-collapse text-center">
          <thead className="h-10 text-sm dark:text-white">
            <tr className="border-b px-4 dark:border-white dark:border-opacity-10">
              <th className="hidden text-center sm:table-cell">{t("watchList.columns.rank")}</th>
              <th className="text-center">{t("watchList.columns.name")}</th>
              <th className="text-center">{t("watchList.columns.price")}</th>
              <th className="hidden text-center md:table-cell">{t("watchList.columns.change24h")}</th>
              <th className="hidden lg:table-cell">{t("watchList.columns.percentage24h")}</th>
              <th className="text-center">{t("watchList.columns.remove")}</th>
            </tr>
          </thead>
            ...
            {coins?.map((coin) => (
              <tr key={coin.id} className="h-20 overflow-hidden border-b text-center dark:border-white dark:border-opacity-10">
                ...
                <td className="hidden text-xs font-medium xs:text-sm md:table-cell">
                  {coin.change24 > 0 ? (
                    <p className="text-green-500 dark:text-green-400">{coin.change24.toFixed(4).toLocaleString("tr-TR")}$</p>
                  ) : (
                    <p className="text-red-500 dark:text-red-500">{coin.change24.toFixed(4)}$</p>
                  )}
                </td>
