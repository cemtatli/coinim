...
import { useTranslation } from "react-i18next"; // i18next kütüphanesini içe aktarın
...
export default function Header() {
  const { t } = useTranslation(); // Dil çevirisi için useTranslation kancasını kullanın
    ...
    <header className="sticky top-0 z-10 flex h-18 w-full items-center justify-center border-b border-dark-primary border-opacity-10 px-4 py-2 dark:border-white dark:border-opacity-10">
      <nav className="flex w-full max-w-[1440px] items-center justify-between">
        <Link to={"/"} className="flex items-center gap-2">
          ...
          <span className="text-lg font-semibold text-blue-500 dark:text-white md:text-xl md:font-bold">{t("appTitle")}</span>
        </Link>
